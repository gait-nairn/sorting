...
def merge_sorted(xs, cmp=cmp_standard):
    ...
    if len(ls) == 0:
        ...
        left = merge_sorted(ls[:len(ls) // 2], cmp)
        right = merge_sorted(ls[len(ls) // 2:], cmp)
...
def quick_sorted(xs, cmp=cmp_standard):
    ...
    if len(ls) == 0:
        ...
        return quick_sorted(less, cmp) + eq + quick_sorted(great, cmp)
